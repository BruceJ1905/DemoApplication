...
@SupportedAnnotationTypes({"com.example.annotation.MyAnnotation"})//标注注解处理器支持的注解类型，就是我们刚才定义的接口Test，可以写入多个注解类型。
...
//        return super.getSupportedAnnotationTypes();
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        TypeSpec helloWorld = TypeSpec.classBuilder("HelloWorld")//创建HelloWorld类
                ...
                .build();
        JavaFile javaFile = JavaFile.builder("com.mytool", helloWorld).build();// 生成源   代码
        try {
